...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            int m = sc.nextInt(); // numero de no terminales
            sc.nextLine();
            ...
            for (int j = 0; j < m; j++) {
                ...
                if (noTerminal.equals("$")) { // verifica si sse ingresa $
                    System.out.println("gramatica no valida, $ no es valido");
                    return;
                }
                List<String[]> listaProducciones = new ArrayList<>();
                for (int k = 1; k < produccion.length; k++) {
                    if (produccion[k].equals("$")) { // verifica si se ingresa $
                        System.out.println("gramatica no valida, $ no es valido");
                        return;
                    }
